...
__all__ = ["AnyError","Response"
           "UserAppDirectory","UserSearchParameters","SemanticSearchAppParameters","GetMergeQueryResults",
           "CreateAppResponse", "SemanticSearchResponse","QuerySession",
           "ResponseQueryInfoListItem","ResponseQueryInfoList","ResponseQueryInfo"
           ]
...
class GetQueryResults(UserSearchParameters):
    ...
    k: int = 1000  # Number of top documents to retrieve
...
class GetMergeQueryResults(GetQueryResults):
    ...
    coefficient_helpers: List[Literal["sub_queries", "keywords", "helper_keywords", "keywords_join_list"]] = Field(
        default_factory=lambda: ["keywords_join_list","sub_queries"], set=True
    ...
    ] = "square_sum"  # Method for merging the search results
...
def get_data(file_path:str, sheet_name:str=None):
    file_extension = os.path.splitext(file_path)[1]
    if file_extension.lower() == '.csv':
        if sheet_name:
            data = pd.read_csv(file_path,sheet_name=sheet_name,encoding="utf-8")
        else:
        ...
        raise "Unsupported file format. File format must be csv or excel"
    return data

def get_sheet_names(file_path):
    """
    Verilen Excel dosyasındaki tüm çalışma sayfası adlarını döndürür.

    :param file_path: Excel dosyasının yolu
    :return: Çalışma sayfası adlarını içeren bir liste
    """
    try:
        # Excel dosyasını yükle
        workbook = load_workbook(file_path)
        # Çalışma sayfası adlarını al
