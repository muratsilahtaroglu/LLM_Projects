...
def main():
    """
    Evaluate generation for a given dataset.

    Args:
        eval_data (str): path to the evaluation json data.
        output_dir (str): The base directory for saving results and logs.
        result_dir (str, optional): The directory relative to output_dir for saving results. Defaults to None.
        min_length (int, optional): How many tokens at minimum for evaluation?. Defaults to 0.
        max_length (int, optional): How many tokens at maximum for evaluation?. Defaults to 100000.
        seed (int, optional): Random seed. Defaults to 42.
        max_num (int, optional): Max number of instances to evaluate. Defaults to None.
        metrics (List[str], optional): List of metrics. {rouge, save_result}. Defaults to [].
    """
    parser = HfArgumentParser([Args])
    args: Args = parser.parse_args_into_dataclasses()[0]

    accelerator = Accelerator(cpu=args.cpu)# canbe use gpu or cpu
    ...
    if not args.enable_tp:
        ...
        # TODO: unwrap because we just use the model for evaluation
