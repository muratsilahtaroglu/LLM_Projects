...
def process_lm_pre(tokenizer, tokenize_max_char=None):
    """
    Pre-processes language modeling data by encoding text using a tokenizer.

    Args:
        tokenizer: The tokenizer to use for encoding the text.
        tokenize_max_char: Optional; If provided, limits the text to a maximum 
                           number of characters before tokenization.

    Returns:
        A function that processes data and returns a dictionary containing 
        'input_ids', which are the encoded representations of the text.
    """
...
def process_lm(tokenizer, max_length=4096, stride=1024, min_length=None):
    ...
    if test[0] == tokenizer.bos_token_id:
        # TODO: subtract 1 because it will be occupied by the bos token
        max_length -= 1
        has_bos = True
    ...
    def _process(data, indices, **kwds):
        ...
        for text, index in zip(data["text"], indices):
            ...
            for start_loc in range(0, seq_len, stride):
                ...
                # TODO: when end_loc is just the same as seq_len, jump out
...
def main():
    ...
    if not args.enable_tp:
        # TODO: if we use deepspeed in evaluation, we must prepare model and dataloader at the same time
