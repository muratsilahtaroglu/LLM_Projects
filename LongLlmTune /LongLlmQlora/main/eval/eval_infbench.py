...
def process_infbench(data, indices, tokenizer, chat_template, task:str, prompt_template:str="mistral", max_length=100000, truncate_from_middle=True):
    """
    Process data for infbench evaluation.

    Args:
        data: The input data.
        indices: The indices of the input data.
        tokenizer: The tokenizer to use.
        chat_template: The chat template to use.
        task: The task name.
        prompt_template: The prompt template to use.
        max_length: The maximum length of the input sequence.
        truncate_from_middle: Whether to truncate the input sequence from the middle.
    ...
    # TODO: high version datasets use LazyBatch to wrap data, which cannot be reverted to list of dicts, thus, we need to convert it to dict first
    ...
    for i, (task, dataset) in enumerate(all_datasets.items()):
        ...
        if args.load_result and os.path.exists(result_path):
            if accelerator.process_index == 0:
                ...
                with open(result_path, encoding="utf-8") as f:
                    ...
                    for line in f:
                        ...
                        label = item["label"]
                        index = item["index"]
                        # TODO: here we explicitly input model_name=None
            ...
            dataloader = DataLoader(
                ...
                pin_memory=not args.cpu, # only pin memory when no gpu
            )
