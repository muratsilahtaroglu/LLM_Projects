...
This project focuses on enhancing LLMs for long-context scenarios, such as processing multi-chapter books, long academic papers, and longitudinal datasets, while reducing computational and memory overhead.

We extend the context length of **Llama-3.1-8B** from 8K to 80K using QLoRA fine-tuning. The training process is highly efficient, requiring only **8 hours on an 8xA800 GPU machine (80G)**. The fine-tuned model demonstrates outstanding performance in long-context tasks such as NIHS, topic retrieval, and long-context language understanding, while maintaining its capabilities in short-context scenarios. 

The context length extension is primarily achieved using only **3.5K synthetic data generated by GPT-4**, showcasing the untapped potential of LLMs to handle extended contexts. With more resources, context length **could exceed 80K**.
...
torchrun --nproc_per_node 8 -m main.eval_longbench --max_length 80000 $COMMAND
