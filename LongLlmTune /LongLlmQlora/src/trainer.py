...
class LLMTrainer(Trainer):
    def __init__(self, *args, model_args, file_logger, **kwargs):
        ...
        Args:
            *args: Arguments passed to Trainer
            model_args (ModelArgs): Model arguments
            file_logger (FileLogger): File logger
    ...
    def _prepare_inputs(self, inputs: Dict[str, Union[torch.Tensor, Any]]) -> Dict[str, Union[torch.Tensor, Any]]:
        ...
        # TODO: reset memory for each individual input
    ...
    def evaluate(self, eval_dataset: Dataset | None = None, ignore_keys: List[str] | None = None, metric_key_prefix: str = "eval") -> Dict[str, float]:        
        ...
        """
        Run evaluation and computes metrics on a dataset.

        Args:
        - eval_dataset: A torch.utils.data.Dataset consisting of tuples of input and target data.
        - ignore_keys: A list of keys in the output dictionary of the model and the metrics that should be ignored when updating the metrics.
        - metric_key_prefix: A string that is used to prefix all keys in the output dictionary of the metrics.
