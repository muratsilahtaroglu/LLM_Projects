...
class Data:
    def _process_language_modeling(data, indices, tokenizer, min_length, max_length):
        outputs = {'input_ids': [], 'attention_mask': [], "labels": [], "length_token": [],"length_text":[], "index": []}
        ...
        for i, text in enumerate(data['text']):
            ...
            # print(len(text))
            text_token_len = len(encoded["input_ids"])
            if  text_token_len < min_length:
                continue
            elif text_token_len < max_length:
                encoded = add_eos(encoded, tokenizer.eos_token_id)
            else:
                n = text_token_len % max_length
            ...
            outputs["length_token"].append(len(encoded['input_ids']))
            outputs["length_text"].append(len(text))
            outputs["index"].append(indices[i])

        return outputs
        
    def _process_language_modeling_2(data, indices, tokenizer, min_length, max_length, overlap=200):
        outputs = {'input_ids': [], 'attention_mask': [], "labels": [], "length_token": [], "length_text": [], "index": []}
        
        def _update_outputs(encoded, outputs, text, i):
            encoded["labels"] = encoded["input_ids"].copy()
            for k, v in encoded.items():
                outputs[k].append(v)
            outputs["length_token"].append(len(encoded['input_ids']))
        ...
        for i, text in enumerate(data['text']):
            ...
            if text_token_len < max_length:
                ...
                encoded = add_eos(encoded, tokenizer.eos_token_id)
                outputs = _update_outputs(encoded, outputs, text, i)
                
            else:
                start = 0
                while start < text_token_len:
                    end = min(start + max_length, text_token_len)
                    chunk = {k: v[start:end] for k, v in encoded.items()}
                    
                    if end < text_token_len:
                        chunk = add_eos(chunk, tokenizer.eos_token_id)
                    outputs = _update_outputs(chunk, outputs, text[start:end], i)
    ...
    def prepare_train_data(data_files=None, tokenizer=None, max_length=4096, min_length=512, chat_template="vicuna", max_sample_num=None, seed=42, cache_dir=None, load_from_cache_file=None):
        ...
        for data_file, max_sample_num in data_2_num_sample.items():
            ...
            if os.path.isdir(data_file) and os.path.exists(os.path.join(data_file, "dataset_info.json")):
                ...
                #max_sample_num= 1000
                #dataset = dataset.select(range(max_sample_num)) #TODO:Murat
