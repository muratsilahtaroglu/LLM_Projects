...
def save_ckpt_for_sentence_transformers(ckpt_dir, pooling_mode: str = 'cls', normalized: bool = True):
    """
    Save a checkpoint for sentence-transformers model given a path to a checkpoint.

    Args:
        ckpt_dir (str): path to the checkpoint to be saved
        pooling_mode (str, optional): pooling mode for the SentenceTransformer model. Defaults to 'cls'.
        normalized (bool, optional): whether to normalize the output embeddings. Defaults to True.
